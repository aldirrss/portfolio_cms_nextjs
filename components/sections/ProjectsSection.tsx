...
export default function ProjectsSection({ showHeader = false }: { showHeader?: boolean }) {
      ...
      <div ref={ref} className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {showHeader && (
          <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}}
            ...
            <div className="font-mono text-xs text-neon-purple/60 tracking-[0.4em] mb-3 uppercase">03. PROJECTS</div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          ...
          {projectCategories.map((cat) => (
            <button key={cat.id} onClick={() => setFilter(cat.id)} data-hover
          ...
          <AnimatePresence mode="popLayout">
            {filtered.map((project, i) => (
              <motion.div key={project.id} layout
                ...
                whileHover={{ y: -6 }}
                className="glass-card overflow-hidden group hover-glow relative" data-hover>

                {project.featured && (
                  <div className="absolute top-3 right-3 z-20 px-2 py-0.5 rounded font-mono text-[10px] tracking-widest"
                ...
                <div className="relative h-48 overflow-hidden">
                  <Image src={project.imageUrl} alt={project.title} fill
                    ...
                    sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" loading="lazy" />
                  <div className="absolute inset-0 opacity-60 transition-opacity duration-300 group-hover:opacity-40"
                    style={{ background: `linear-gradient(to bottom, ${project.color}10, rgba(13,20,36,0.8))` }} />
        ...
        <motion.div initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}} transition={{ delay: 0.5 }}
          ...
          <a href="https://github.com" target="_blank" rel="noopener noreferrer"
            className="btn-secondary inline-flex items-center gap-2" data-hover>
            ◈ View All on GitHub
          </a>
        </motion.div>
      </div>
